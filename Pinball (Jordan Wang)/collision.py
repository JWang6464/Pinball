...
Professor Harper
04/18/2023
This module contains methods that account for the colisions of objects
within a pinball machine. Overall, this program consists of a several
differnt components that ultimately make a pinball machine simulation.
In order to run this code, the user can enter collision.py into the
terminal or press the run button in vs code.
'''
...
def buildObstacles(win):
    ...
    btmleftborder = pho.Block(win, 8, 2, 13, 9, color = (10, 10, 10))
    btmrightborder = pho.Block(win, 8, 2, 27, 9, color = (10, 10, 10))
    obs1 = pho.Triangle(win, x0 = 18 , y0 = 17, color = (255, 64, 64))
    obs2 = pho.Ball(win, x0 = 27, y0 = 30, color = (138, 43, 226))
    obs2.setElasticity(1.15)
    obs3 = pho.Ball(win, x0 = 15, y0 = 25, color = (134, 43, 20))
    obs3.setElasticity(1.15)
    obs4 = pho.Triangle(win, x0 = 26, y0 = 20, color = (152, 245, 255))
    obs4.setElasticity(1.15)
    obs5 = pho.Block(win, 2, 3, 15, 35, color = (255, 97, 3))
    obs6 = pho.Block(win, 2, 5, 23, 35, color = (127, 255, 0))
    obs7 = pho.Block(win, 1, 2, 20, 28, color = (255, 185, 15))
    obstacles = [leftborder, rightborder, topborder, btmleftborder, btmrightborder, obs1, obs2, obs3, obs4, obs5, obs6, obs7]
    # Return the list of Things
    return obstacles

def main():
    '''This is the main function of the code that brings all elements together'''
    win = gr.GraphWin("Pinball Simulation", 500, 500)
    ...
    #ball.draw()
    molecule = pho.Molecule(win)
    molecule.setRadius(0.8)
    molecule.setColor([150,150,150])
    molecule.setVelocity(random.randint(-10, 10), 10)
    molecule.setAcceleration(0, 0)
    molecule.setPosition(17,6)
    molecule.draw()

    new_obs_list =[]
    
    print("Press n to create a new obstacle")
    print("Use arrow keys to move object")
    ...
    while True:
        ...
        if keypressed == "q":
            break
        if keypressed =="n":
            newobs = pho.Block(win, color = (118, 238, 198))
    ...
    # if the ball is out of bounds, re-launch it
        ...
        for item in shapes:
            ...
            if collision(molecule, item, dt):
                collided = True
            
        # if collided is equal to False
        if collided == False:
            # call the update met`hod of the ball with dt as the time step
            molecule.update(dt)
            # newball.update(dt)
        # increment frame
        frame += 10
